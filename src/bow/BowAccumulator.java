...
public class BowAccumulator {
    ...
    public void addBow(Map<String, MerchantDetails> uniqueMerchantsToUpdate) {
        ...
        List<String> merchantIds = (List<String>) uniqueMerchantsToUpdate.keySet();
        //get factual bow
        ...
        List<YelpResult> yelpResults = yelpManager.getYelpResults(yelpQueries, true);


        assert merchantIds.size() == yelpResults.size();
        //add yelp bow to uniqueMerchantsToUpdate
        for (int i=0; i<merchantIds.size(); i++)
            uniqueMerchantsToUpdate.get(merchantIds.get(i)).getBow().setYelp(yelpResults.get(i).getCategories());
